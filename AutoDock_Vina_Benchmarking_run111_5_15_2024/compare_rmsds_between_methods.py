...
#also make a csv file that has the rmsd for motif and autodock in each line per system. List motif first, autodock second
rmsds_out_file = open("all_rmsds.csv", "w")
rmsds_out_file.write(",Motifs,Autodock\n")
for system in motif_set.keys():
	if system in autodock_set.keys():
		#print(system)

		rmsds_out_file.write(system + "," + str(motif_set[system]) + "," + str(autodock_set[system]) + "\n")
